...
动过三维、主线步骤或界面布局，再跑一次冒烟。它跑的是构建产物，要排在 `check:code` 之后，
浏览器内核也得先装一次（[docs/DEVELOPMENT.md](docs/DEVELOPMENT.md#命令)）：

```bash
npm run smoke
```

CI 上每个 PR 只跑 `check:code`，冒烟要合并进 `main` 之后才跑（[为什么](docs/DEVELOPMENT.md#检查)），所以本地这一遍不能省。
...
- **加一步、加一个模块**照 [docs/DEVELOPMENT.md](docs/DEVELOPMENT.md#加一步) 的清单走，别漏了 `cam.fit`。
- **改样式先改令牌**，组件不直接写颜色和字号；文案按 [docs/UI.md](docs/UI.md#文案) 的规矩写。
- **旁白只有一处出处**：主线在步骤的 `narration` 字段，模块在 `src/modules/vo.js`。
- **生成物别手改**，清单与重出命令见 [docs/DEVELOPMENT.md](docs/DEVELOPMENT.md#生成物)。
...
PR 描述照模板填：改了什么、为什么、怎么验证的。改动涉及画面的话，附一张截图。
...
双轨授权，界线与商业条款见 [COMMERCIAL.md](COMMERCIAL.md)。

**提交 PR 即表示**：你的贡献以同样的双轨条件发布，并且你同意版权人可以把它包含在另行商业授权的版本里。
只要有一段贡献没有这个许可，商业授权就给不出去。
