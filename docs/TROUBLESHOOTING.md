...
按「你看到了什么」找。控制台里 `__ctx` 是全片共享上下文，`__engine` 是分步引擎。
...
三维初始化失败，九成是 WebGL 不可用：

1. 打开 <https://get.webgl.org/webgl2/>，看这台机器支不支持 WebGL 2；
2. 不支持的话，换一个基线之内的浏览器（[DEVELOPMENT.md](DEVELOPMENT.md#环境)）；
3. 支持却仍然失败，看控制台第一条红色报错，封面上只是它的结果。

远程桌面、虚拟机和部分 Linux 发行版默认禁用硬件加速，也会落到这一条。Chrome 可以用 `--enable-unsafe-swiftshader` 强制走软件渲染，慢但能跑。

### 封面一直停在「正在……」，进度条不走

某一段初始化卡住了。`main()` 里 `await` 到的拒绝都会被外层 `.catch()` 接住并换成「三维画面没能启动」，
所以停在「正在……」只可能是接不住的两类：没被 `await` 的浮动 Promise，以及主循环 updater 里抛出的错（各有一层 try-catch，只打日志）。
控制台通常有一条未捕获异常。另外确认 `node_modules/three` 存在，且版本是 0.185.x。

### 一片黑 / 一片空白，控制台没有报错

多半是 canvas 尺寸为 0。`#stage` 是 `position: fixed; inset: 0`，需要视口有真实高度；检查是不是把页面嵌进了一个高度未定的容器。
...
```js
__ctx.stage._lastFrame     // 这一步实际用的机位声明；fit 为 undefined 就是漏了
__ctx.stage.safe           // 界面占掉的上下边（像素）
```

取值见 `src/steps/util.js` 的 `FIT_*`，或直接写 `{ r: 水平半径, h: 垂直半高 }`（毫米，相对镜头目标点）。

`fit` 写了仍裁边，量一下这一步真正有多大。拆开、离位陈列、装饰件外移都会让实际跨度远超「一盏灯」：
...
还有一种：算出来的距离被手动缩放的上限夹住了。`setRecommended()` 会把 `maxDistance` 抬到至少容得下这一步，绕开它直接写 `controls.maxDistance` 就会复发：
...
是 `0px` 而底部确实有一排控件，分两种：那段 HTML 的根节点没有 `.dock` 类（`hud.dock()` 生成的结构不要在 `onMount` 里替换掉）；
或者 `#syncSafe()` 里写 `--dock-h` 那一句被挪到了「安全区没变就早退」之后。坞高与安全区是两件事，坞比底部那一摞矮时安全区不变，早退就再也写不到这个变量。

### 自己写脚本截图，机位怎么设都不生效

封面挂着一个每帧改写推荐机位的自转 updater（`main.js` 的 `drift`）。把 `#cover` 直接 `hidden` 掉它照样在跑，之后任何 `setRecommended()` 都会被逐帧覆盖。
...
`tools/shots.mjs` 就是这么做的。开场那一步在远处撒了一圈「灯河」光点精灵，拍产品图时关掉，否则画面边角会留下几点橙色：
...
光加多了。`src/render/stage.js` 的 `MOODS` 里四路光的总辐照只留了约三成余量，越过就被 ACES 推平。
想让画面更亮，改 `bg`（背景那一对颜色）与 `bloom`，别整体上调光（[DESIGN.md §7](DESIGN.md#7-画面曝光与背景)）。

### 浅色模式下整盏灯像蒙了一层白纱

浅色三档（craft / studio / dusk）的 `bloom` 被改成了非零。键光下的绵纸亮度贴着高通阈值，走 composer 的档位上纸面会自己越线起辉，
辉光糊回棂条与窗花；低配档走直出，所以从来没有这层白。把 `MOODS.light` 各档的 `bloom` 改回 0。深色与夜色的溢出留给灯焰，不在此列。

阈值本身由 `setMood()` 取「0.86 与本档背景最亮处 + 0.02 的高者」，绕过它直接写 `stage.bloom.threshold` 会让背景渐变被高通裁出一枚有边沿的白斑：
...
`Machining._orientTool()` 搭的正交基取反了，或者被改回了 `lookAt()`。`npm run smoke` 每一步都验刃口与攻角的点积，反了直接报 `dot=-1.000`。
...
探面探空了。刀的高度是每帧探出来的：沿刀身取五处，各从上方沿进刀方向打射线，取最高的面，把刃尖放上去。
`carve` 漏了、构件当时还没摆到位（`detach()` 排在 `begin()` 之后）、或者传的是一件隐藏着的备份件，`rideMeshes` 就是空的，
刀直接坐在走刀线上，而走刀线是走到底时刃尖的位置，不是料的表面。
...
反过来，**刀停在面上不往里走**多半不是毛病：口子比刀窄（透眼 4 mm，与凿身同宽）时，刚体本来就进不去。

### 格心与木条的接缝在明暗闪烁（拖拽旋转后尤其明显）

两个面落在同一个平面上，深度缓冲里是同一个值。把所有材质的深度函数从 `LessEqual` 换成 `Less` 再渲一帧，两次之间会变的像素就是深度完全打平的地方：
...
正常只剩个位数的零星像素。成片出现说明 `lattice.js` 里那 0.3 mm 的渲染让位（`GAP`）被拿掉了，或者新加的构件也做成了严丝合缝
（[DESIGN.md §1](DESIGN.md#1-几何算出来的不是建出来的)）。

`near` 别调大：它能换来深度精度，但 `minDistance` 量的是相机到目标的距离，拉到最近时相机已经探进灯笼里，手机上的 D5 最近处只有 7.3 mm。

### 拖着转的时候，木头（或压在灯笼纸上的棂条）在沸腾

程序化花纹的走样：`materials.js` 的 `fbmF()` 被换回了不带足迹的 `fbm()`，或者足迹参数（`fw`）算错了，比如拿世界坐标而不是构件自己的坐标去求导
（[DESIGN.md §1](DESIGN.md#1-几何算出来的不是建出来的)）。

判据是「同一处像素在相邻两帧之间来回跳」，不是「整片在变」：镜头转起来，花纹本来就该跟着木头一起挪。把木料换成平色跑一遍，量出「合法的明暗变化」有多少：
...
木料着色器的取样点被改回了世界坐标。`vGrain` 应当取 `position`，即构件自己的坐标系：料被拖动、离位陈列、爆炸拆开都不能让花纹动，
转过来摆的料也不能变成横纹。十三根各有各的花靠 `seed` 与 `tone`，不靠位置。

### 明明什么都停了，画面却在轻微颤动

`stage.js` 里 `setSafeArea()` 的 `REFRAME_MIN` 门槛被拿掉了：字幕换一行，安全区就差一行的高度，每变一次就重算一次机位，
相机与主光的目标点跟着挪，阴影贴图整体位移，灯笼纸上棂条的影子持续爬动（[DESIGN.md §6](DESIGN.md#6-取景算出来的不是摆出来的)）。
...
补齐 `attachAll()` / `showOnly()` / `allFinished()` / `showPanels` / `showDecor` / 灯芯这几项（[DEVELOPMENT.md](DEVELOPMENT.md#一步长什么样)）。
`npm run smoke` 会倒着走一遍十八步，并数每一步画面里有几件东西。

### 教学件的凹槽看起来是凸的

那处用实心块拼了。改用 `demoSolid({ blank, cuts })` 让 CSG 真的挖出来，内壁才会照常受光（[DESIGN.md §4](DESIGN.md#4-教学件与刀具让动作和结果对得上)）。

### 浅色模式下某几步仍然是暗的

设计如此：夜里的场景（D5 与其后的互动模块）不跟主题走（[UI.md](UI.md#主题)）。切回白天的场景时自己恢复，菜单里的选择不受影响。
开场的傍晚（A1）是跟主题走的，浅色下偏米色属正常。

### 帧率低 / 风扇狂转

`src/render/fx.js` 的 `detectTier()` 分三档，移动设备一律判成 low（内存与核心数只管桌面端）。
low 档像素比压到 1.5、离屏目标不开 MSAA、关阴影；mid 档像素比 1.75、MSAA 2×、阴影贴图 1024，见 `src/render/stage.js` 的 `TIERS`。
另有两项不在表里：关高光溢出在 `main.js`，M4 天球贴图减半在 `modules/m3-m4.js`。
...
1. 右上角菜单里「声音」是否开着；
2. 浏览器要求一次真实手势才允许出声，页面上任何一次真实点击或按键都能解锁，脚本里直接调 `__engine.go()` 不算；
3. 系统或标签页是否静音。

「旁白朗读」只在真的有配音文件时才出现在菜单里。仓库不含音频，放进 `public/audio/vo/` 并在同目录 `manifest.json` 里登记之后它才会出现。字幕一直都在。
...
`vite.config.js` 的 `base` 被从 `'./'` 改成了绝对路径，产物不再是相对路径，子路径部署当场 404。
