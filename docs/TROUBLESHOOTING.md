...
`setMood()` 会把阈值取成「作者定的 0.86」与「本档背景最亮处 + 0.02」的高者，所以复发只有一种可能：
...
两个面落在同一个平面上了，深度缓冲里是同一个值 —— 谁盖住谁由浮点末位决定。
不必靠肉眼判：把所有材质的深度函数从 `LessEqual` 换成 `Less` 再渲一帧，
两次之间会变的像素就是深度完全打平的地方。
