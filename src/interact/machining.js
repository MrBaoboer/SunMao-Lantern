 ...
 * 刃尖跟随料面的每帧限速（毫米）。
 *
 * 探面的结果会**整段整段地跳**：刀跨过孔沿时，五处里「最高的那一处」从料面
 * 换成了走刀线，一下差出十来毫米。按比例缓（指数平滑）压不住这种跳变 ——
 * 比例调到能压住，平时又跟不上面。限速则两头都对：小变化原样通过，
 * 大跳变匀速走过去，屏幕上就没有那一记台阶。
 */
const RIDE_MAX_STEP = 0.35;
...
export class Machining {
   ...
   * 把刀放到当前料面上。
   *
   * 沿刀身工作端取五处（刃尖、前后两端与两处中点），各从上方打一条射线到工件上，取**最高**的那个面 ——
   * 刚性的刀身架在最高处，底下有坑就悬着，绝不会有一处陷进实料。
   * 五处都打空（刀已经走出料外）时沿用上一次的高度，免得在空中跳一下。
   *
  ...
  _ride(at) {
    ...
    /*
     * 探面的结果会跳台阶，直接用就是刀在料面上一上一下地颠。
     *
     * 两个原因叠在一起：几何按 1/32 量化重建，切削面本身是一级一级的；
     * 而五处取最高，刀往前挪一点，「最高的那一处」就可能整个换成另一处 ——
     * 跨过孔沿时一帧能差十来毫米。第二趟走刀最明显。
     * 所以限速跟随（见 RIDE_MAX_STEP）。开工那一下（传了 at）仍然直接站定。
     */
    if (at === undefined) {
      const d = off - j.rideLast;
      off = j.rideLast + Math.max(-RIDE_MAX_STEP, Math.min(RIDE_MAX_STEP, d));
    }
    j.rideLast = off;
