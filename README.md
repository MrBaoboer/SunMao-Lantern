...
<img src="docs/img/hero.png" width="880" alt="做好的榫卯灯笼：四面万字纹格心糊着绵纸，四角立柱与出头的榫，一颗钉子也没有" />
...
| **做骨架** | 开槽、切榫、凿透眼、凿柱窝，十三根木条合龙 |
| **装点年味** | 选花纹、装格心、糊纸贴花，最后拆开看一遍 |

做完之后还有四件事：点灯、猜灯谜、写心愿、挂灯笼。
...
- **几何是算出来的。** 基本模数 a = 12 mm，所有尺寸都是 a 或 a/12 的整数倍，整套几何跑在整数毫米栅格上。十三件木构件由「毛坯盒减去若干带工序标签的切除盒」生成 —— 开槽、凿透眼、凿柱窝、切榫、开颈，每道工序都是一次精确的布尔减法。于是加工感不用画（CSG 标出哪些面是新切出来的，着色器据此提亮一档），穿模可以证伪（十七件两两求交，136 组配对重叠体积全为 0），面数也压得住（十七件包络合计 1152 个三角形）。
- **料是跟着刀走的。** 凿子压在哪条槽上，就只有那条槽在掉料；深度按刀数一层层推进，手往回拖木头也不会长回去。做法是把「刀的位置 + 走了多少」直接喂给 CSG，现算这一刀啃掉的那一块 —— 不是走完几刀之后把整道工序一次性开出来。工具是锯、凿、刨三样：柱窝和细颈本来就是凿的活，装板槽是刨的活。
- **取景是算出来的。** 每一步只声明「必须完整看到多大一块」，相机自己算该退到多远：水平与垂直各算一次取远的那个，界面占掉的画面也一并算进去。刀身也算进去 —— 凿自刃口向上还有 60 mm。同一份内容，1440×900 与 390×844 上都完整。
- **音效是实时合成的。** 因为它必须可参数化 ——「第二记高 2 个半音」「四根立柱到位音依次上行」，固定采样做不到。声音只有三类来源：木头、纸、火。
- **纹样是程序化的。** 格心有两种纹样，都由一组「二维线段 + 线宽」生成，再挤出成真实的镂空棂条：万字纹（默认）把卍字连续铺陈，四臂钩转落在单元角点上，相邻单元自然接续；麻叶纹是六边形棂格加每格六出放射。同一份线段数据同时用于灯上的棂条、地面的光斑和海报的脚线 —— 你选的纹样，三处永远是同一个。
...
需要动手的步骤，底部中央会出现一个按钮；构件直接拖就行，每一次装配都只有一个合法方向 —— 呼吸的小箭头会指出来是哪个方向。
不想自己动手，旁边一直有「帮我加工」「帮我装上」—— 走同一段动画、同一组音效，跳过的只是手感。

**不拦人。** 旁白没念完可以往前翻，任务没做完也可以往前翻。对一个自愿点进来的教学页面，「被拦住」的伤害远大于「少练一次」。
...
本文档里的四张图也是从构建产物里拍的，模型或界面改动之后重出一遍即可，图与实现不会各说各话：
