...
<img src="docs/img/hero.png" width="880" alt="点亮的榫卯灯笼，格心纹样在地面投出一片光斑" />
...
`npm run verify` 会在 Node 端跑完 15 条断言；页面加载时也会跑一遍，界面右上角的「尺寸对照」可以随时调阅。

### 取景是算出来的，不是摆出来的

每一步只声明「这一步必须完整看到多大一块」，相机自己算该退到多远。

竖屏手机的水平视场只有十几度，按笔记本调好的机位横过来一定裁边 ——
所以水平与垂直两个方向各算一次取远的那个，界面占掉多少画面也一并算进去：
底部摊开一排卡片时，灯笼会自己往上让、往后退。同一份内容，1440×900 与 390×844 上都完整。
...
**环境**：Node 18 以上。运行时只有一个依赖 [three.js](https://threejs.org)；
